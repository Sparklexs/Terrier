...
public abstract class BaseMatching implements Matching
{
	...
	/**
	 * A property that enables to ignore the terms with a low IDF. Corresponds
	 * to property <tt>ignore.low.idf.terms</tt>. Defaults to true. This can
	 * cause some query terms to be omitted in small corpora.
	 */
	...
	protected void initialisePostings(MatchingQueryTerms queryTerms)
	{
		...
		for (String queryTerm : queryTermStrings)
		{
			...
			if (t != null)
			{
				...
				if (IGNORE_LOW_IDF_TERMS
				        && collectionStatistics.getNumberOfDocuments() < t
				                .getFrequency())
				{
				...
				queryTermsToMatchList.add(new MapEntry<String, LexiconEntry>(
				        queryTerm, t));
			}
			else
		...
		// logger.warn("queryTermsToMatchList = " +
		// queryTermsToMatchList.size());
		int queryLength = queryTermsToMatchList.size();

		wm = new WeightingModel[queryLength][];
		for (int i = 0; i < queryLength; i++)
		{
			Map.Entry<String, LexiconEntry> termEntry = queryTermsToMatchList
			        .get(i);
			...
			// if none were provided with the query we seek the entry statistics
			// query term in the lexicon
			if (entryStats == null)
			{
				entryStats = lexiconEntry;
				// save them as they may be useful for query expansion. HOWEVER
				// ONLY IF we didnt
	...
	protected void initialise(MatchingQueryTerms queryTerms)
	{
		...
		IGNORE_LOW_IDF_TERMS = Boolean.parseBoolean(ApplicationSetup
		        .getProperty("ignore.low.idf.terms", "true"));
	...
	protected void finalise(MatchingQueryTerms queryTerms)
	{
		...
		logger.debug("Number of docs with +ve score: "
		 ...
		 * application dependent dsms, then it's up to the application, ie YOU!
		 * to ensure that the BooleanFallback is applied last.
		 */

		/*
		 * dsms each require resorts of the result list. This is expensive, so
		 * should be avoided if possible. Sorting is only done if the dsm
		 * actually altered any scores
		 */

		/*
		 * query dependent modification of scores of documents for a query,
		...
		/*
		 * application dependent modification of scores of documents for a
		 * query, based on a static set by the client code sorting the result
		 * set after applying each DSM
		 */
		for (int t = 0; t < numOfDocModifiers; t++)
		{
			if (documentModifiers.get(t).modifyScores(index, queryTerms,
			        resultSet))
				resultSet.sort(resultSet.getResultSize());
		}
		logger.debug("number of retrieved documents: "
