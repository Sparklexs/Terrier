...
public class FSOMapFileLexiconOutputStream extends LexiconOutputStream<String>
{
	        ...
	        throws IOException
	{
		...
		mapFileWriter = FSOrderedMapFile.mapFileWrite(FSOMapFileLexicon
		        .constructFilename(structureName, path, prefix,
		                FSOMapFileLexicon.MAPFILE_EXT));
		keyFactory = _keyFactory;
		tempKey = keyFactory.newInstance();
	}

	@SuppressWarnings("unchecked")
	static FixedSizeWriteableFactory<Text> getKeyFactory(Index _index,
	        String _structureName) throws IOException
	{
		_index.addIndexStructure(_structureName + "-keyfactory",
		        org.terrier.structures.seralization.FixedSizeTextFactory.class
		                .getName(), "java.lang.String", "${max.term.length}");
		_index.flush();
		return (FixedSizeWriteableFactory<Text>) _index
		        .getIndexStructure(_structureName + "-keyfactory");
	}
	...
	public FSOMapFileLexiconOutputStream(
	        ...
	        throws IOException
	{
		this(_index.getPath(), _index.getPrefix(), _structureName,
		        getKeyFactory(_index, _structureName));
		this.index = _index;
	...
	@Override
	public int writeNextEntry(String _key, LexiconEntry _value)
	        throws IOException
	{
		tempKey.set(_key);
	...
	{
		index.addIndexStructure(structureName + "-valuefactory",
		        leValueClassname, "", "");
