 ...
 * <b>Properties:</b>
 * <ul>
 * <li><tt>metaindex.compressed.max.data.in-mem.mb</tt> - maximum size that a
 * meta index .zdata file will be kept in memory. Defaults to 400(mb).</li>
 * <li><tt>metaindex.compressed.max.index.in-mem.mb</tt> - maximum size that a
 * meta index .zdata file will be kept in memory. Defaults to 100(mb).</li>
 * <li><tt>metaindex.compressed.reverse.allow.duplicates</tt> - set this
 * property to true to suppress errors when a reverse meta value is not unique.
 * Default false.</li>
 * <li><tt>metaindex.compressed.crop.long</tt> - set this property to suppress
 * errors with overlong Document metadata, while will instead be cropped.</li>
 * 
 * @since 3.0
        ...
        Flushable
{
	...
	protected static final int	                MAX_MB_IN_MEM_RETRIEVAL	               = Integer
	                                                                                           .parseInt(ApplicationSetup
	                                                                                                   .getProperty(
	                                                                                                           "metaindex.compressed.max.data.in-mem.mb",
	                                                                                                           "400"));
	protected static final int	                MAX_INDEX_MB_IN_MEM_RETRIEVAL	       = Integer
	                                                                                           .parseInt(ApplicationSetup
	                                                                                                   .getProperty(
	                                                                                                           "metaindex.compressed.max.index.in-mem.mb",
	                                                                                                           "100"));
	...
	protected static final boolean	            CROP_LONG	                           = Boolean
	                                                                                           .parseBoolean(ApplicationSetup
	                                                                                                   .getProperty(
	                                                                                                           "metaindex.compressed.crop.long",
	                                                                                                           "false"));
	...
	protected static final int	                ZIP_COMPRESSION_LEVEL	               = 5;	                                                  // TODO
																																			  // (auto)configure?
	 ...
	 * @param _forwardKeys
	 */
	public CompressingMetaIndexBuilder(IndexOnDisk _index, String[] _keyNames,
	...
	public CompressingMetaIndexBuilder(IndexOnDisk _index,
	        String _structureName, String[] _keyNames, int[] _valueLens,
	        String[] _forwardKeys)
	{
		this.index = _index;
		...
		try
		{
			this.dataOutput = new DataOutputStream(Files.writeFileStream(_index
			        ...
			        + structureName
			        + ".zdata"));
			this.indexOutput = new DataOutputStream(
			        Files.writeFileStream(_index.getPath() + "/"
			                + _index.getPrefix() + "." + structureName + ".idx"));
		}
	...
	public void writeDocumentEntry(String[] data) throws IOException
	{
		...
		for (i = 0; i < forwardKeys.length; i++)
		{
			...
			if (lastValues[i] != null
			        && data[forwardKeys[i]].compareTo(lastValues[i]) < 1)
	...
	public void close() throws IOException
	{
		...
		index.setIndexProperty("index." + structureName + ".data-source",
		        currentOffset > MAX_MB_IN_MEM_RETRIEVAL * (long) 1024
		                * (long) 1024 ? "file" : "fileinmem");
		index.setIndexProperty("index." + structureName + ".index-source",
		        currentIndexOffset > MAX_INDEX_MB_IN_MEM_RETRIEVAL
		                * (long) 1024 * (long) 1024 ? "file" : "fileinmem");
		// TODO emit warnings
		index.flush();
	...
	public static void main(String[] args) throws Exception
	{
		...
		{
			System.err
			        .println("Usage: -Dterrier.index.path=hdfs://path/to/index "
			                + CompressingMetaIndexBuilder.class.getName()
			                + " docno");
			return;
		}
	 ...
	 * @throws Exception
	 */
	public static void reverseAsMapReduceJob(IndexOnDisk index,
	        String structureName, String[] keys) throws Exception
	{
	...
	// @SuppressWarnings("deprecation")
	public static void reverseAsMapReduceJob(IndexOnDisk index,
	        String structureName, String[] keys, HadoopPlugin.JobFactory jf)
	        throws Exception
	{
		...
		conf.set("MetaIndexInputStreamRecordReader.structureName",
		        structureName);
		...
		conf.set(
		        "CompressingMetaIndexBuilder.forward.valueLengths",
		        index.getIndexProperty("index." + structureName
		                + ".value-lengths", ""));
		conf.set("CompressingMetaIndexBuilder.forward.keys", index
		        .getIndexProperty("index." + structureName + ".key-names", ""));
		...
		// only update the index from the controlling process, so that we dont
		// have locking/concurrency issues
		index.setIndexProperty("index." + structureName + ".reverse-key-names",
		        ArrayUtils.join(keys, ","));
		index.flush();
	        ...
	        Partitioner<KeyValueTuple, IntWritable>
	{
		 ...
		 * {@inheritDoc}
		 */
		public int getPartition(KeyValueTuple kv, IntWritable docid,
		        int numReducers)
		{
			if (numReducers == 1)
				return 0;
	        ...
	        HadoopUtility.MapReduceBase<IntWritable, Wrapper<String[]>, KeyValueTuple, IntWritable, Object, Object>
	{
		...
		protected void configureMap() throws IOException
		{
			reverseKeyCount = jc.getInt(
			        "CompressingMetaIndexBuilder.reverse.keyCount", 0);
			reverseKeyNames = jc.get(
			        "CompressingMetaIndexBuilder.reverse.keys", "").split(
			        "\\s*,\\s*");
			final TObjectIntHashMap<String> key2forwardOffset = new TObjectIntHashMap<String>(
			        reverseKeyCount);
			final String[] forwardKeyNames = jc.get(
			        "CompressingMetaIndexBuilder.forward.keys", "").split(
			        "\\s*,\\s*");
			int i = 0;
		...
		public void map(IntWritable docid, Wrapper<String[]> _metadata,
		        OutputCollector<KeyValueTuple, IntWritable> collector,
		        Reporter reporter) throws IOException
		{
			String[] metadata = _metadata.getObject();
			reporter.setStatus("Processing metadata for document "
			        + docid.get());
			for (int i = 0; i < reverseKeyCount; i++)
			{
				collector.collect(new KeyValueTuple(reverseKeyNames[i],
				        metadata[i]), docid);
			}
			reporter.progress();
		}
		...
		protected void configureReduce() throws IOException
		{
			...
			final String[] allKeys = index.getIndexProperty(
			        "index." + structureName + ".key-names", "").split(
			        "\\s*,\\s*");
			final String[] allValueLens = index.getIndexProperty(
			        "index." + structureName + ".value-lengths", "").split(
			        "\\s*,\\s*");
			i = 0;
		...
		/**
		 * Reduce function. Input Key: (meta Key name, meta Key value) Value:
		 * list of matching docids.
		 */
		public void reduce(KeyValueTuple metaTuple,
		        Iterator<IntWritable> docids,
		        ...
		        throws IOException
		{
			        ...
			        || !metaTuple.getKeyName().equals(currentReducingKey))
			{
				...
				logger.info("Opening new MapFileWriter for key "
				        + currentReducingKey);
			}
			...
			reporter.setStatus("Reducing metadata value "
			        + metaTuple.getValue());
		}
		...
		protected void closeReduce() throws IOException
		{
			...
			{
				logger.info("Finished reducing for " + currentReducingKey
				        + ", with " + currentKeyTupleCount + " entries");
			}
			if (duplicateKeyCount > 0)
			{
				logger.warn("MetaIndex key "
		        ...
		        throws IOException
		{
			...
			logger.info("Opening MapFileWriter for key " + keyName
			        + " - index " + metaKeyIndex);
			 ...
			 * reduceTaskFileDestinations.toString() + "/" + index.getPrefix() +
			 * "." + jc.get("MetaIndexInputStreamRecordReader.structureName") +
			 * "-"+metaKeyIndex+FSOrderedMapFile.USUAL_EXTENSION,
			 * REVERSE_KEY_LOOKUP_WRITING_BUFFER_SIZE, keyFactory = new
			 * FixedSizeTextFactory(valueLength), new
