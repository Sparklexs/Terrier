 ...
 * <ul>
 * <li><tt>tokeniser</tt>, the tokeniser class to be used (defaults to
 * EnglishTokeniser);</li>
 * <li><tt>max.term.length</tt>, the maximum length in characters of a term
 * (defaults to 20);</li>
 * <li><tt>lowercase</tt>, whether characters are transformed to lowercase
 * (defaults to <tt>true</tt>).</li>
 * <li><tt>TaggedDocument.abstracts</tt> - names of the abstracts to be saved
 * for query-biased summarisation. Defaults to empty. Example:
 * <tt>TaggedDocument.abstracts=title,abstract</tt></li>
 * <li><tt>TaggedDocument.abstracts.tags</tt> - names of tags to save text from
 * for the purposes of query-biased summarisation. Example:
 * <tt>TaggedDocument.abstracts=title,body</tt>. ELSE is special tag name, which
 * means anything not consumed by other tags.</li>
 * <li><tt>TaggedDocument.abstracts.lengths</tt> - max lengths of the asbtrcts.
 * Defaults to empty. Example:
 * <tt>TaggedDocument.abstracts.lengths=100,2048</tt></li>
 * <li><tt>TaggedDocument.abstracts.tags.casesensitive</li> - should the names
 * of tags be case-sensitive? Defaults to false.</li>
 * </ul>
 * 
 * @author Craig Macdonald, Vassilis Plachouras, Richard McCreadie, Rodrygo
 *         Santos
 * @since 3.5
 */
public class TaggedDocument implements Document
{
	...
	/** Change to lowercase? */
	protected final static boolean	lowercase	              = Boolean
	                                                                  .parseBoolean(ApplicationSetup
	                                                                          .getProperty(
	                                                                                  "lowercase",
	                                                                                  "true"));
	...
	/**
	 * The tags to process exactly. For these tags, the check() method is not
	 * applied.
	 */
	...
	protected final String[]	    abstractnames	          = ArrayUtils
	                                                                  .parseCommaDelimitedString(ApplicationSetup
	                                                                          .getProperty(
	                                                                                  "TaggedDocument.abstracts",
	                                                                                  "")
	                                                                          .toLowerCase());
	/** The fields that the named abstracts come from (comma separated list) **/
	protected final String[]	    abstracttags	          = ArrayUtils
	                                                                  .parseCommaDelimitedString(ApplicationSetup
	                                                                          .getProperty(
	                                                                                  "TaggedDocument.abstracts.tags",
	                                                                                  ""));
	/** The maximum length of each named abstract (comma separated list) **/
	protected final int[]	        abstractlengths	          = ArrayUtils
	                                                                  .parseDelimitedInts(
	                                                                          ApplicationSetup
	                                                                                  .getProperty(
	                                                                                          ...
	                                                                                          ""),
	                                                                          ",");
	protected final boolean	        abstractTagsCaseSensitive	= Boolean
	                                                                  .parseBoolean(ApplicationSetup
	                                                                          .getProperty(
	                                                                                  "TaggedDocument.abstracts.tags.casesensitive",
	                                                                                  "false"));
	...
	public TaggedDocument(InputStream docStream,
	        Map<String, String> docProperties, Tokeniser _tokeniser,
	        String doctags, String exactdoctags, String fieldtags)
	{
		...
		try
		{
			...
			this.br = new BufferedReader(
			        charset != null ? new InputStreamReader(docStream, charset)
			                : new InputStreamReader(docStream));
		}
		catch (UnsupportedEncodingException uee)
		{
			logger.warn("Desired encoding (" + charset
			        + ") unsupported. Resorting to platform default.", uee);
			this.br = new BufferedReader(new InputStreamReader(docStream));
		}
		this.properties = docProperties;

		if (doctags != null)
			this._tags = new TagSet(doctags);
		else
			this._tags = new TagSet(TagSet.TREC_DOC_TAGS);

		if (exactdoctags != null)
			this._exact = new TagSet(exactdoctags);
		else
			this._exact = new TagSet(TagSet.TREC_EXACT_DOC_TAGS);

		if (fieldtags != null)
			this._fields = new TagSet(fieldtags);
		else
	 ...
	 * 
	 * @param docReader
	 *            Reader the stream from the collection that ends at the end of
	 *            the current document.
	 */
	public TaggedDocument(Reader docReader, Map<String, String> docProperties,
	...
	/**
	 * Returns the underlying buffered reader, so that client code can tokenise
	 * the document itself, and deal with it how it likes.
	 */
	...
	protected final StringBuilder	sw	      = new StringBuilder(
	                                                  tokenMaximumLength);
	 ...
	 * 
	 * @return String the next token of the document, or null if the token was
	 *         discarded during tokenisation.
	 */
	public String getNextTerm()
	{
		...
		while (btag && ch != -1 && !EOD)
		{
			// initialise the stringbuffer with the maximum length of a term
			// (heuristic)
			...
			try
			{
				...
				if (ch == '<')
				{
					...
					{ // else if it is a comment, that is <!
						counter++;
						ch = br.read();
						if (ch == '[')
						{
				...
				if ((btag = (tag_close || tag_open)))
				{
					...
					while (ch != -1 && ch != '<' && ch != '>')
					{
						...
						if (!endOfTagName && Character.isWhitespace((char) ch))
						{
							...
							// System.err.println("Found tag  " + tagName +
							// (tag_open ? " open" : " close") );
					...
					{
						tagName = tagNameSB.toString();
						// System.err.println("Found tag " + tagName+ (tag_open
						// ? " open" : " close"));
						tagNameSB.setLength(0);
					}
				}
				else
				{ // otherwise, if we are not in the body of a tag
				...
				if (tag_open)
				{
					...
					if ((_tags.isTagToProcess(tagName) || _tags
					        .isTagToSkip(tagName)) && !tagName.equals(""))
					{
			...
			{
				logger.warn(
				        "Input/Output exception during reading tokens. Document "
				                + this.getProperty("docno"), ioe);
				return null;
			}
		}
		if (ch == -1)
		...
		if (!btag && (!hasWhitelist || (hasWhitelist && inTagToProcess))
		        && !inTagToSkip)
		{
	...
	protected void processEndOfDocument()
	{
		...
		{
			setProperty(abstractnames[abstractId], abstracts[abstractId]
			        .toString().trim());
		}
	}
	 ...
	 * text or some subset of it should be saved. The default behaviour checks
	 * each abstract named in TaggedDocument.absracts, if for an abstract we are
	 * in the correct field (specified in TaggedDocument.abstracts.tags) and
	 * then it saves up to maximum character length specified in
	 * TaggedDocument.abstracts.lengths.
	 * 
	 * The 'ELSE' abstract tag is a special case that will be filled with any
	 * tag that is not added to an existing abstract.
	 * 
	 * TaggedDocument should be sub-classed and this method overwritten if you
	 * want to save abstracts in a different manner, e.g. saving the first
	 * paragraph.
	 * 
	 * @param text
	...
	protected void saveToAbstract(String text, String tag)
	{
		...
		for (int i = 0; i < abstractCount; i++)
		{
			if (abstracttags[i].equals(tag))
			{
				...
				if (currentAbstractLength < maxAbstractLength)
				{
					...
					{
						abstracts[i].append(' ');
						abstracts[i].append(text.substring(0, maxAbstractLength
						        - currentAbstractLength));
					}
				}
			}
		}
		if (elseAbstractSpecialTag != -1 && !tagFound)
		{
			...
			final int currentAbstractLength = abstracts[elseAbstractSpecialTag]
			        .length();
			final int textLength = text.length();
			// abstract >= maxAbstractLength
			// abstract + text <= maxAbstractLength
			...
			if (currentAbstractLength < maxAbstractLength)
			{
				...
				{
					abstracts[elseAbstractSpecialTag].append(' ');
					abstracts[elseAbstractSpecialTag].append(text.substring(0,
	 ...
	 * 
	 * @return HashSet a hashset containing the fields that the current term
	 *         appears in.
	 */
	...
	/**
	 * Indicates whether the tokenizer has reached the end of the current
	 * document.
	 * 
	 * @return boolean true if the end of the current document has been reached,
	...
	protected void processEndOfTag(String tag)
	{
		...
		{ // else report an error, and find the tag.
			int _counter = 0;
	...
	/**
	 * The maximum number of consecutive same letters or digits that are allowed
	 * in valid terms.
	 */
	protected static final int	maxNumOfSameConseqLettersPerTerm	= 3;

	/**
	 * Checks whether a term is shorter than the maximum allowed length, and
	 * whether a term does not have many numerical digits or many consecutive
	 * same digits or letters.
	 * 
	 * @param s
	...
	public static String check(String s)
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			ch = chNew;
			/*
			 * if it contains more than 3 consequtive same letters (or digits),
	 ...
	 * 
	 * @param name
	 *            Name of the property. It is suggested, but not required that
	...
	/**
	 * Allows a named property to be added to the Document. Examples might be
	 * URL, filename etc.
	 * 
	 * @param name
	 *            Name of the property. It is suggested, but not required that
	...
	/**
	 * Returns the underlying map of all the properties defined by this
	 * Document.
	 * 
	 * @since 1.1.0
	 */
	...
	public static Document generateDocumentFromFile(final String filename)
	{
		...
		{
			logger.fatal("ERROR: Problem opening TRECDocument test file : "
			        + ioe);
