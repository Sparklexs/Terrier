 ...
 * 
 * This application recompress an inverted index using the specified compression
 * configuration.
 * 
 * Usage: InvertedIndexRecompresser
 * 
 * The terrier.properties must contain the following properties:
 * 
 * indexing.tmp-inverted.compression.configuration=
 * IntegerCodecCompressionConfiguration e.g.
 * indexing.tmp-inverted.compression.configuration=
 * {@link IntegerCodecCompressionConfiguration}
 * 
 * If you are targetting IntegerCodecCompressionConfiguration, you probably need
 * the following properties:
 * 
 * compression.integer.chunk.size=N
 * compression.tmp-inverted.integer.ids.codec=the {@link IntegerCodec} to use to
 * compress ids compression.tmp-inverted.integer.ids.codec=the
 * {@link IntegerCodec} to use to compress tfs
 * compression.tmp-inverted.integer.ids.codec=the {@link IntegerCodec} to use to
 * compress fields (if any) compression.tmp-inverted.integer.ids.codec=the
 * {@link IntegerCodec} to use to compress blocks (if any)
 * 
 * For example:
 * 
 ...
 * indexing.tmp
 * -inverted.compression.configuration=IntegerCodecCompressionConfiguration
...
public class InvertedIndexRecompresser
{
	...
	static void compressInverted(IndexOnDisk index,
	        AbstractPostingOutputStream icpw, Set<String> queryTerms)
	        throws IOException
	{
		...
		Iterator<Entry<String, LexiconEntry>> iterator = (Iterator<Entry<String, LexiconEntry>>) index
		        .getIndexStructureInputStream("lexicon");

		PostingIndexInputStream iiis = (PostingIndexInputStream) index
		        .getIndexStructureInputStream("inverted");

		FixedSizeWriteableFactory<LexiconEntry> valueFactory = (FixedSizeWriteableFactory<LexiconEntry>) index
		        .getIndexStructure("lexicon-valuefactory");
		Class<? extends FixedSizeWriteableFactory<LexiconEntry>> valueFactoryClass = (Class<? extends FixedSizeWriteableFactory<LexiconEntry>>) valueFactory
		        .getClass();

		LexiconOutputStream<String> los = new FSOMapFileLexiconOutputStream(
		        index, "newlex", valueFactoryClass);

		TerrierTimer tt = new TerrierTimer("Recompressing inverted index",
		        index.getCollectionStatistics().getNumberOfPointers());
		tt.start();
		try
		{
			// int cnt = 0;
			while (iterator.hasNext())
			{
				...
				if (queryTerms == null || queryTerms.contains(lee.getKey()))
				{
					...
					BitIndexPointer bitPointer = icpw
					        .writePostings(postingList);
					...
					if (queryTerms != null)
						logger.debug("Saving " + lee.getKey() + "...");
				}
				tt.increment();
			}
		}
	...
	{
		CompressionConfiguration compressionConfig = CompressionFactory
		        .getCompressionConfiguration("tmp-inverted", ArrayUtils
		                .parseCommaDelimitedString(index.getIndexProperty(
		                        "index.inverted.fields.names", "")), index
		                .getIntIndexProperty("index.inverted.blocks", 0) > 0);
		logger.info("Recompressing inverted structure using "
		        + compressionConfig.toString());

		AbstractPostingOutputStream icpw = compressionConfig
		        .getPostingOutputStream(index.getPath()
		                + ApplicationSetup.FILE_SEPARATOR + index.getPrefix()
		                + "." + "tmp-inverted"
		                + compressionConfig.getStructureFileExtension());
		...
		compressionConfig.writeIndexProperties(index,
		        "lexicon-entry-inputstream");
		logger.info("New inverted file size: "
		        + Files.length(index.getPath()
		                + ApplicationSetup.FILE_SEPARATOR + index.getPrefix()
		                + "." + "tmp-inverted"
		                + compressionConfig.getStructureFileExtension()));
